...
do for [i=0:maxStep/100:timeStep/100] {
    ...
    set ylabel "P/GPa"
    set yrange [0:25]
    plot sprintf('./output/Type_5_%d.dat', i) u 4:($8*1e-9) with lines lw 2 lt rgb "black"

    set origin 0.5, 0.48
    set title 'Density'
    set ylabel "ρ/kg·m^{-3}"
    set yrange [1.2e3:2.4e3]
    plot sprintf('./output/Type_5_%d.dat', i) u 4:7 with lines lw 2 lt rgb "black"

    set origin 0, 0
    set title 'Velocity'
    set ylabel "v/m·s^{-1}"
    set yrange [0:2e3]
    plot sprintf('./output/Type_5_%d.dat', i) u 4:5 with lines  lw 2 lt rgb "black"#, \
       # sprintf('./output/Type_5_%d.dat', i) u 4:5 with points pt 1 ps 0.5 lt rgb "black"
